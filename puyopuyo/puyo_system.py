...
class GraphicBoard:
    ...
    def __init__(self):
        ...
        self.__check_dic = {
            self.__check_list[0]: 0,
            self.__check_list[1]: 1,
            self.__check_list[2]: 2,
            self.__check_list[3]: 3,
        }
    ...
    def __check_chain(self, x, y, color):
        for check_grid in self.__check_list:
            ...
            if (0 <= c_x < self.board_x) and (0 <= c_y < self.board_y):
                if self.__board[c_y][c_x] == color and not (
                    self.__contain_puyo_list([color, c_x, c_y], self.removing_puyo_list)
                ):
